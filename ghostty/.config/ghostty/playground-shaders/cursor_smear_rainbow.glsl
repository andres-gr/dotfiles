const float normValue = 0.7;
const float durationValue = 0.35;
const float fadeValue = 1.25;
...
vec2 norm(vec2 value, float isPosition) {
    ...
    return (value * normValue - (iResolution.xy * isPosition)) / iResolution.y;
}
...
const float DURATION = durationValue; //IN SECONDS
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    float fadeFactor = fadeValue - smoothstep(lineLength, sdfCurrentCursor, easedProgress * lineLength);
